...
const useDraggableContainer = (
    ...
    const items = ref<Array<any>>([...originalItems.value])

    // onUpdated(() => {
    //     console.log('in hook, update---')
    //     console.log('items:', items.value.length)
    //     console.log('originalItems:', originalItems.value.length)
    // })
    // update items while props.list changed
    watch(
        () => originalItems.value,
        newValue => {
            console.log('---watch originalItems.value')
    ...
    const onItemDragOver = ({ position }: { position: number }) => {
        ...
        console.log('onDragOver, items.length:', items.value.length)
